...
def get_component(app):
    ...
    @app.callback(Output('battery', 'children'), [Input('battery-update', 'n_intervals')])
    ...
    return html.Div([
        ...
        html.H3(
            f"GPU: {other_hardware.get_gpu()}, Placa-mãe: {other_hardware.get_motherboard()} "),

        html.H3(
            f'Usuário: {other_hardware.get_user_name()}  // PID Login: {other_hardware.get_user_pid()}'),
        html.H3(id="battery")
    ])
